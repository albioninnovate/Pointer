...
This script is a server running in micropython for the PICOW.  Its primary function is on connection from a client 
 ...
 all these need to be flashed to the PI CO in addition to this script:
    bno055.py
    bno055_base.py
    secrets.py
    wifi.py
...
# open a Access point to which clients can connect
# the gateway IP adress is '192.168.4.1'
ap = network.WLAN(network.AP_IF)
ap.config(essid='scope')
ap.config(password='scopescope')
ap.active(True)

# to logon to a wifi net work uncomment, not needed if connecting direclty to an AP on the pico server
#wifi.main()
